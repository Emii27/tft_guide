...
import { FlakAccuracy, FlakType, TargetVisibility, Weather } from '~/types/enum'

const flakAccuracies = Object.values(FlakAccuracy)
...
const flakType = useState('flakType')
const flakAccuracy = useState('flakAccuracy', () => FlakAccuracy.Inaccurate)

const { data } = await useFetch('/api/flak/accuracy')
...
const flakTypeModifier = computed(() => {
  switch (flakType.value) {
    case FlakType.Barrage:
      return '-1 if "Barrage" type flak is used'
    case FlakType.ContinuePointFire:
      return '+1 if "Continuously Pointed Fire" type flak is used'
    default:
      return null
  }
})
    ...
    <div class="row">
      <div>
        ...
        <div class="px-4 py-3 border-t border-accented">
          ...
          >
            <USelect
              v-model="flakAccuracy"
              :items="flakAccuracies"
              class="min-w-32"
            />
          </UFormField>
        </div>
      ...
      >
        <div v-if="!isOverTargetZone">
          ...
          <ul>
            ...
            <li v-if="flakType != FlakType.Barrage">
              -1 if bomber is performing "Evasive Action"
            </li>
